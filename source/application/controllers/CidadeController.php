...
class CidadeController extends Zend_Controller_Action
{
	...
	public function selectAction()
	{
		...
		if (!$codUf || $codUf=="")
		{
			$arrOpcoes[] = array("id"=>"", "nome"=>". selecione um estado .");
		}
		else
		{
			$tblCidade = new Cidade();
			$rsCidade = $tblCidade->busca(array("bln_ativo="=>1,
													"int_idfuf="=>$codUf),
									array("int_idacidade"));
									
			foreach ($rsCidade as $cidade)
			{
				$arrOpcoes[] = array("id"=>$cidade->int_idacidade,
										"nome"=>$cidade->vhr_nome);
			}
		}
